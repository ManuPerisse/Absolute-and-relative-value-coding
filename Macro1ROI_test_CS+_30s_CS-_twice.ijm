macro "1ROI_4odourpresentations" {
...
sum = 0;
for (i=from1; i<to1; i++) sum = sum + f[i];
f01 = sum / nAvg1;

// Calculate average over frames specified by from-to 2
sum = 0;
for (i=from2; i<to2; i++) sum = sum + f[i];
f02 = sum / nAvg2;

// Calculate average over frames specified by from-to 3
sum = 0;
for (i=from3; i<to3; i++) sum = sum + f[i];
f03 = sum / nAvg3;

// Calculate average over frames specified by from-to 4
sum = 0;
for (i=from4; i<to4; i++) sum = sum + f[i];
f04 = sum / nAvg4;
...
// Calculate DF/F 3
dff3 = newArray(numSlices);
for (i=0; i<nSlices; i++) dff3[i] = (f[i] - f03) / f03;
// Calculate DF/F 4
dff4 = newArray(numSlices);
for (i=0; i<nSlices; i++) dff4[i] = (f[i] - f04) / f04;

// Print
for (i=0; i<nSlices; i++) {
print(dff1[i]+"	"+dff2[i]+"	"+dff3[i]+"	"+dff4[i]);
String.append(dff1[i]+"	"+dff2[i]+"	"+dff3[i]+"	"+dff4[i]+"\n");
};
selectWindow("Log");
waitForUser("Done");
run("Close All");
